...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  if (message.type === 'GET_PAGE_CONTENT') {
    ...
    chrome.tabs.query({ active: true, currentWindow: true }, async (tabs) => {
      if (!tabs[0]) {
        sendResponse({ success: false, error: '当前没有可用的活动标签页' });
        return;
      }
...
chrome.runtime.onInstalled.addListener(async (details) => {
  if (details.reason === 'install') {
    ...
    // 扩展更新时仅补齐缺失的内置 Prompt，不覆盖用户已修改内容
    chrome.storage.sync.get(['prompts'], (result) => {
      const prompts = result.prompts || [];
      const mergedPrompts = mergeMissingBuiltinPrompts(prompts, getDefaultPrompts());
...
function mergeMissingBuiltinPrompts(existingPrompts, builtinPrompts) {
  const existingIds = new Set(existingPrompts.map(prompt => prompt.id));
  const missingBuiltinPrompts = builtinPrompts.filter(prompt => !existingIds.has(prompt.id));
  return [...existingPrompts, ...missingBuiltinPrompts];
}
